...
build_frontend() {
  (cd frontend && BACKEND_URL=http://127.0.0.1:8000 NEXT_TELEMETRY_DISABLED=1 npm run build 2>&1 | tail -2 | sed 's/^/    /')
}
...
apply() {   # $1=舊 commit（none＝第一次）、$2=新 commit；依變更檔案決定要做什麼
  ...
  if grep -qE '^backend/(pyproject\.toml|requirements\.lock)$' <<<"$changed"; then
    log "  後端依賴有變 → pip install"; (cd backend && (.venv/bin/pip install -q -r requirements.lock || true) && .venv/bin/pip install -q -e ".[llm]")
  fi
  if grep -qE '^frontend/package(-lock)?\.json$' <<<"$changed"; then
    log "  前端依賴有變 → npm ci"; (cd frontend && npm ci --no-audit --no-fund --silent)
  fi
  local did_build=0
  if grep -qE '^frontend/' <<<"$changed"; then
    log "  前端有變 → next build"; build_frontend; did_build=1
  fi
...
ExecStart=/usr/bin/flock -n /tmp/ntpc-autodeploy.lock ${APP}/deploy/autodeploy.sh run
EOF
...
EOF
  ...
  run)
    ...
    old="$(git rev-parse HEAD 2>/dev/null || echo none)"; new="$(git rev-parse "origin/$BRANCH")"
    [[ "$old" == "$new" ]] && exit 0

...
- [ ] **樹林區地籍圖**（樹德段、太平段、文林段；比準地 樹德段1415、比較標的 284／367、917／317 與 P001～P004 區段內宗地）：向主辦要 GeoJSON／Shapefile，或以地政局名義申請國土測繪中心地籍查詢 API（段代碼已可查：樹林區 F17）。有了才能自動算面積、寬深、形狀、臨街、位置。
- [ ] **比準地樹德段1415 登記面積**（無交易紀錄，實價登錄查不到）：地籍謄本或主辦提供。
- [ ] **新北市樹林區土地平均區段地價表（住宅區）110、111 年**：表4 期日調整 5.96%／4.09%／5.49% 的依據（不是都市地價指數）；拿到後在 `rules/` 加一份指數表並讓 `market/index.py` 可切換來源。
- [ ] 地價區段圖 P001～P004（沒有就用四至文字圍面，標草稿；啟智街187巷24弄 路網缺，P001 西南角需人工點）。

...
for(const anchor of document.querySelectorAll('a[href^="#"]'))anchor.addEventListener('click',event=>{const section=document.getElementById(anchor.getAttribute('href').slice(1));if(section){event.preventDefault();section.scrollIntoView({behavior:reduced?'instant':'smooth',block:'start'});history.replaceState(null,'',anchor.getAttribute('href'));}});

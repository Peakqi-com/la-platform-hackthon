...
function StatusCell({ cell }: { cell: string }) {
  return (<span className="inline-flex flex-wrap gap-1">{statusTokens(cell).map((t, i) => <span key={i} className={`inline-block border rounded px-1.5 py-0.5 text-[10px] leading-tight whitespace-nowrap ${BADGE[statusKind(t)]}`}>{t}</span>)}</span>);
}
...
function Table({ headers, rows, active }: { headers: string[]; rows: string[][]; active: Set<StatusKind> }) {
  ...
  if (!shown.length) return <div className="text-xs text-slate-500 py-3 border-t-2 border-[#2a1d14]">這一章沒有符合篩選的項目。</div>;
  const w = (i: number) => {
    const h = headers[i];
  ...
  return (
    <div className="overflow-x-auto">
      <table className="src-table w-full text-xs">
        <thead><tr>{headers.map((h, i) => <th key={i} className={`${w(i)} text-left font-normal text-slate-500 text-[11px] pb-2 pr-3 align-bottom`}>{h}</th>)}</tr></thead>
        <tbody>
          {shown.map((r, ri) => (
            <tr key={ri}>
              {headers.map((_, ci) => {
                ...
                if (ci === ni) { const { title, note } = splitName(cell); return <td key={ci} className="align-top py-3 pr-3"><div className="font-semibold leading-snug"><Inline text={title} /></div>{note && <div className="text-[11px] text-slate-500 mt-0.5 leading-snug"><Inline text={note} /></div>}</td>; }
...
function BlockView({ b, active }: { b: Block; active: Set<StatusKind> }) {
  if (b.kind === "p") return <p className="text-xs text-slate-700 leading-relaxed mb-3 max-w-3xl"><Inline text={b.text} /></p>;
  if (b.kind === "list") return <ul className="text-xs text-slate-700 leading-relaxed list-disc pl-5 mb-3 max-w-3xl space-y-1">{b.items.map((it, i) => <li key={i}><Inline text={it} /></li>)}</ul>;
  ...
  return (
    <div className="sources">
      <header className="mb-8">
        <div className="text-[10px] font-semibold tracking-wider text-[#c2410c] mb-2">土地徵收補償市價查估 · 估價案件審查輔助系統</div>
        <h1 className="src-serif text-2xl font-bold leading-tight mb-3">查估審查系統資料來源總表</h1>
        {doc?.intro.map((t, i) => <p key={i} className="text-xs leading-relaxed text-slate-700 max-w-3xl mb-1"><Inline text={t} /></p>)}
      ...
      {!doc && !err && <div className="text-xs text-slate-500">載入中…</div>}

      {doc && (
        <div className="flex gap-10 items-start">
          <aside className="no-print w-44 shrink-0 sticky top-4">
            <div className="text-[11px] text-slate-500 mb-2">目錄</div>
            <ol className="border-l border-slate-300">
              {numbered.map((s) => (
                <li key={s.id}><a href={`#${s.id}`} className="flex items-baseline gap-3 pl-4 py-1 text-xs hover:text-[#c2410c]"><span className="text-slate-400 w-4 text-center">{s.numeral}</span><span>{s.title}</span><span className="text-[11px] text-slate-400">{s.rowCount || ""}</span></a></li>))}
            </ol>
            <div className="mt-6 pt-4 border-t border-slate-200">
              <div className="text-[11px] text-slate-500 mb-2">只看</div>
              <div className="flex flex-wrap gap-1.5">
                {STATUS_KINDS.map((k) => <button key={k.kind} type="button" aria-pressed={active.has(k.kind)} onClick={() => toggle(k.kind)} className={`border rounded-full px-2 py-0.5 text-[11px] ${BADGE[k.kind]} ${active.has(k.kind) ? "ring-2 ring-offset-1 ring-[#ea580c]" : "opacity-80"}`}>{k.label}</button>)}
              </div>
              <div className="text-[10px] text-slate-500 mt-2 leading-relaxed">再按一次取消。未按任何一個時顯示全部。</div>
            </div>
          </aside>
          ...
          <div className="flex-1 min-w-0">
            {numbered.map((s) => (
              <section key={s.id} id={s.id} className="mb-12 scroll-mt-4">
                <h2 className="src-serif text-lg font-bold mb-2 flex items-baseline gap-3"><span className="font-mono text-xs font-normal text-[#c2410c]">{numberOf(s)}</span>{s.title}</h2>
              ...
              <div className="mt-4 pt-6 border-t-2 border-[#2a1d14]">
                {appendix.map((s) => (
                  <section key={s.id} id={s.id} className="mb-8 scroll-mt-4">
                    <h2 className="src-serif text-base font-bold mb-2">{s.title}</h2>
                    {s.blocks.map((b, i) => <BlockView key={i} b={b} active={active} />)}
                  </section>))}
              </div>
            )}

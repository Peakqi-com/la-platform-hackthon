...
/* 書表預覽：照《查估書表範本》六頁順序（表1、表5、表4、區段略圖、使用分區圖、地價區段圖）。
...
export default function Sheets() {
  ...
  if (!rec) return <Empty />;
  const name = (t: string) => (t === "表1" ? "地價區段勘查表" : t.startsWith("表5") ? "影響地價區域因素分析明細表" : "比較法調查估價表");
  return (
    <div>
      <div className="sheets-page-head"><PageHeader title="② 產出書表" desc="六頁書表依範本頁序重算，畫面所見即下載的 Excel 與 PDF；列印請由 PDF 進行（三表 A4、三圖 A3 橫式，剛好六頁）。"
      ...
      <div className="no-print flex flex-wrap items-center gap-2 mb-3 bg-white border border-slate-200 rounded-lg px-3 py-2">
        ...
        <div className="ml-auto flex gap-2">
          ...
          <Btn onClick={() => download("pdf")} disabled={stale || !!busy} busy={busy === "pdf"} title={stale ? "產出已過期，請先重新產生書表" : "六頁 PDF，照範本頁序"}>{busy === "pdf" ? "產生中…" : "⬇ 下載完整書表 PDF"}</Btn>
          <Btn kind="ghost" onClick={() => window.open(`/api/cases/${encodeURIComponent(rec.id)}/sheets.pdf?inline=1`, "_blank")} disabled={stale} title={stale ? "產出已過期，請先重新產生書表" : "在新分頁開啟六頁 PDF（三表＋三圖，照範本頁序與紙張），從 PDF 列印"}>🖨 列印（六頁 PDF）</Btn>
        </div>
      </div>
